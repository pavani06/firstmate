...
The deterministic diff pass that every mode's definition of done renders is none of those: it is the worker's own self-report of diff facts, not an added reviewer, a clean verdict, or a gate, and it replaces no existing authority ([`docs/deterministic-review.md`](docs/deterministic-review.md)).
