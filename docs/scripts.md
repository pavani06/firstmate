...
| `fm-diff-review.sh`      | Emit a line-anchored hunk coverage skeleton for one PR or task diff, then assert a typed change/no-change claim and the diff's required text, paths and file count against it (docs/deterministic-review.md) |
