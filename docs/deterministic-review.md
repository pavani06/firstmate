...
Fleet PR review has a deterministic, LLM-free layer that asserts structural properties of a task diff before or alongside agent reasoning.
...
The port keeps the validated parsing semantics exactly, and `tests/fm-diff-review.test.sh` pins them.
...
At least one assertion must be requested, because a review run with nothing to assert is a caller mistake.
...
The future adoption path is a separate graduation decision that would integrate the delegation skeleton (`preview` + `rule`) into the fleet review flow as the deterministic scaffold under the hosting agent's reasoning; until then, the `fm-diff-review.sh` assertions above are the adopted deterministic layer.
